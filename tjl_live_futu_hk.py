...
ATR_SL       = 1.0     # tuned: 1× ATR stop loss
ATR_TP       = 1.5     # tuned: 1.5× ATR take profit (3:2 R:R)
ATR_PERIOD   = 14
...
def check_tjl_model_e(price, highs, lows, closes, volumes, today_high):
    ...
    vol_ok    = vol_ratio >= 1.0    # tuned: 1.0× (was 1.5×)

    above_high = price > high_20
    below_low  = price < low_20

    long_fire  = above_high and vol_ok and (rsi > 40)   # tuned: RSI 40 (was 50)
    short_fire = below_low  and vol_ok and (rsi < 60)   # tuned: RSI 60 (was 50)
...
def check_tjl_model_f(price, highs, lows, closes, volumes, today_high, today_low):
    ...
    avg_vol20 = np.mean(volumes[-21:-1])
    vol_ratio = volumes[-1] / avg_vol20 if avg_vol20 > 0 else 0
    vol_ok    = vol_ratio >= 1.0    # tuned: 1.0× (was 1.2×)

    price_above_ema = price > ema20
    price_below_ema = price < ema20

    # RSI crossed above 60 — tuned from 50
    rsi_cross_up   = (prev_rsi is not None) and (prev_rsi < 60) and (rsi > 60)
    # RSI crossed below 45 — tuned from 50
    rsi_cross_down = (prev_rsi is not None) and (prev_rsi > 45) and (rsi < 45)
...
def check_tjl_model_g(price, highs, lows, closes, volumes, today_high, today_low, today_open):
    ...
    vol_ok    = vol_now >= vol_avg20 * 1.0    # tuned: 1.0× (was 1.2×)
...
def check_tjl_model_i(price, highs, lows, closes, volumes, today_high):
    ...
    # Pullback zone: price within 5% of WMA63 — tuned from 3% (wider = more signals)
...
def check_tjl_model_j(price, highs, lows, closes, volumes, today_high):
    ...
    # Pullback: price within 3% of 150-DMA — tuned from 2% (wider = more signals)
    near_dma150 = abs(price - dma150) / dma150 <= 0.03
